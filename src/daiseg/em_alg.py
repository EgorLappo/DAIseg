...
def train_em_normalized(
    O1, O2, L1, L2, init_lmbd, rr, Ti, init_a, max_iter=20, tol=1e-4
):
    M, N = O1.shape
    ...
    for it in range(max_iter):
        ...
        print(
            f"  Iter {it + 1}: LL={log_lik:.2f}, Delta={diff:.4f} | Rates: N={new_lmbd[0]:.5f}, AF={new_lmbd[1]:.5f}, I={new_lmbd[2]:.5f}"
        )
...
def run_daiseg_em(json_file):
    ...
    obs_seq, nst = hmm.create_observations(
        data["data"], data["window_callability"]["Thousand_genomes"]
    )
    prms = data["parameters_initial"]
    gen_time, mu, rr, l = (
        prms["generation_time"],
        prms["mutation"],
        prms["rr"],
        prms["window_length"],
    )
...
def run_batch_em_pipeline(
    json_files_list, output_combined_file=None, max_iter=15, tol=1e-6
):
    """
    ...
    for j_file in json_files_list:
        ...
        obs_seq, _ = hmm.create_observations(
            ...
            data["prefix"] + "/" + data["window_callability"]["Thousand_genomes"],
        )
        ...
        cal_nd_1kG = np.loadtxt(
            data["prefix"] + "/" + data["window_callability"]["Nd_1k_genomes"],
            usecols=-1,
        )
        O1, O2, names = hmm.prepare_matrices_from_dict(obs_seq)
        M, N = O1.shape
    ...
    for it in range(max_iter):
        ...
        for item in batch_data:
            ...
            nums, dens, ll = e_step_normalized(
                emit_linear, trans_linear, start_linear, O1, O2, L1, L2
            )
        ...
        print(
            f"Iter {it + 1}: LL={iter_log_lik:.2f} | Rates: N={new_lmbd[0]:.5f}, AF={new_lmbd[1]:.5f}, I={new_lmbd[2]:.5f}"
        )
    ...
    for item in batch_data:
        ...
        with open(output_tsv, "w", encoding="utf-8") as f:
            ...
            for sample_name, tracks in out_dict_new.items():
                ...
                for start, end in archaic_intervals:
                    f.write(
                        f"{sample_name}\t{jsn_data['CHROM']}\t{start}\t{end}\t{end - start + 1}\n"
                    )
    ...
    if output_combined_file:
        ...
        df = pd.DataFrame(
            all_results, columns=["Sample", "CHR", "Start", "End", "Length"]
        )
