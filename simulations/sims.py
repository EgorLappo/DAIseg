...
def history_archaic(prms, ne, t, p_admix, seed):
    ...
    demography.add_population_parameters_change(
        time=0, initial_size=ne["eu"], population="EU", growth_rate=0.00202
    )
    demography.add_population_parameters_change(
        time=t["t_eu_growth"],
        initial_size=ne["eu_growth"],
        population="EU",
        growth_rate=0,
    )
    ...
    demography.add_population_split(
        time=t["t_ooa"], derived=["AF", "OOA"], ancestral="AMH"
    )
    demography.add_population_split(
        time=t["t_amh"], derived=["AMH", "ND"], ancestral="ANCES"
    )
...
def get_migrating_tracts_ind(ts, pop_name, ind_node, T_anc):
    ...
    for p in ts.populations():
        if p.metadata.get("name") == pop_name:
            ...
            break
    if pop_id == -1:
        raise ValueError(f"Pop '{pop_name}' not found")
...
def generate_haplotype_table(ts, chrom_name, assign_nucleotides=True):
    ...
    if n_vars == 0:
        return pd.DataFrame(
            columns=["#CHROM", "POS", "REF", "ALT", "Ancestral", "Outgroup", "Neand"]
        )
    ...
    data = {
        "#CHROM": [chrom_name] * n_vars,
        "POS": pos,
        "REF": refs,
        "ALT": alts,
        "Ancestral": refs,
        "Outgroup": fmt_col(i_af),
        "Neand": fmt_col(i_nd),
    }
    ...
    for n in i_eu:
        ind = n_ind[n]
        if ind != -1:
            eu_map.setdefault(ind, []).append(n)
...
def process_one_chromosome(seed, prms, ne, t, p_admix, out_dir):
    ...
    def get_ids(nm):
        ...
        return sorted(
            [
                int(i)
                for i in set(ts.nodes_individual[ts.samples(population=pid[0])])
                if i != -1
            ]
        )
    ...
    cfg = {
        ...
        "parameters_initial": {
            "admixture_proportion": p_admix,
            "introgression_time": int(t["t_nd_migration"] * prms["gen_time"]),
            "rr": prms["recomb_rate"],
            "mutation": prms["mut_rate"],
            "window_length": 1000,
            "generation_time": int(prms["gen_time"]),
            "t_archaic_c": int(t["t_amh"] * prms["gen_time"]),
            "t_split_c": int(t["t_ooa"] * prms["gen_time"]),
            "t_introgression_c": int(t["t_nd_migration"] * prms["gen_time"]),
...
def run_daiseg_task(seed, out_dir):
    ...
    if os.path.exists(res_file):
        try:
            ...
            if not tmp.empty:
                if "CHR" not in tmp.columns:
                    tmp.insert(0, "CHR", seed)
...
def calculate_accuracy(true_pth, inf_pth):
    ...
    return {
        "True_BP": int(t_bp),
        "Inferred_BP": int(i_bp),
        "Overlap_BP": int(overlap),
        "Precision": round(p, 4),
        "Recall": round(r, 4),
        "F1": round(f1, 4),
    }
